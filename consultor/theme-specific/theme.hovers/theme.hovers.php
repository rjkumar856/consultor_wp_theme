...
if (!function_exists('consultor_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'consultor_hovers_theme_setup3', 3 );
	function consultor_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		consultor_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'consultor'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'consultor') ),
				"std" => 'slide_left',
				"options" => array(
					'default'		=> esc_html__('Fade',				'consultor'),
					'slide_left'	=> esc_html__('Slide from Left',	'consultor'),
					'slide_right'	=> esc_html__('Slide from Right',	'consultor'),
					'slide_top'		=> esc_html__('Slide from Top',		'consultor'),
					'slide_bottom'	=> esc_html__('Slide from Bottom',	'consultor'),
//					'arrow'			=> esc_html__('Arrow',				'consultor'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'consultor'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'consultor') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'consultor'),
					'icon'	=> esc_html__('Icon',	'consultor'),
					'icons'	=> esc_html__('Icons',	'consultor'),
					'zoom'	=> esc_html__('Zoom',	'consultor'),
					'fade'	=> esc_html__('Fade',	'consultor'),
					'slide'	=> esc_html__('Slide',	'consultor'),
					'pull'	=> esc_html__('Pull',	'consultor'),
					'border'=> esc_html__('Border',	'consultor')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('consultor_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'consultor_hovers_theme_setup9', 9 );
	function consultor_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'consultor_hovers_frontend_scripts', 1010 );
		add_filter( 'consultor_filter_localize_script','consultor_hovers_localize_script' );
		add_filter( 'consultor_filter_merge_scripts',	'consultor_hovers_merge_scripts' );
		add_filter( 'consultor_filter_merge_styles',	'consultor_hovers_merge_styles' );
		add_filter( 'consultor_filter_get_css', 		'consultor_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'consultor_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'consultor_hovers_frontend_scripts', 1010 );
	function consultor_hovers_frontend_scripts() {
		if ( consultor_is_on(consultor_get_theme_option('debug_mode')) && consultor_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'consultor-hovers', consultor_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( consultor_is_on(consultor_get_theme_option('debug_mode')) && consultor_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'consultor-hovers',  consultor_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('consultor_hovers_merge_scripts')) {
	//Handler of the add_filter( 'consultor_filter_merge_scripts', 'consultor_hovers_merge_scripts' );
	function consultor_hovers_merge_scripts($list) {
...
if (!function_exists('consultor_hovers_merge_styles')) {
	//Handler of the add_filter( 'consultor_filter_merge_styles', 'consultor_hovers_merge_styles' );
	function consultor_hovers_merge_styles($list) {
...
if (!function_exists('consultor_hovers_localize_script')) {
	//Handler of the add_filter( 'consultor_filter_localize_script','consultor_hovers_localize_script' );
	function consultor_hovers_localize_script($arr) {
		$arr['button_hover'] = consultor_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('consultor_hovers_add_icons') ) {
	function consultor_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
				...
				if (!is_object($args['cat']) && $product->is_purchasable() && $product->is_in_stock()) {
					consultor_show_layout(apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'consultor') : '')
										. '</a>',
										$product));
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'consultor');
						else
							esc_html_e('Details', 'consultor');
					}
				?></a>
			</div>
			<?php
		...
		// Hover style 'Icon'
			...
			?><div class="icons"><a href="<?php the_permalink(); ?>" aria-hidden="true" class="icon-resize-full-alt"></a></div><?php
...
// Add styles into CSS
if ( !function_exists( 'consultor_hovers_get_css' ) ) {
	//Handler of the add_filter( 'consultor_filter_get_css', 'consultor_hovers_get_css', 10, 4 );
	function consultor_hovers_get_css($css, $colors, $fonts, $scheme='') {
...
.sc_button_hover_style_hover.sc_button_hover_slide_left {		background: linear-gradient(to right,	{$colors['text_hover']} 50%, {$colors['alter_bg_color']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['alter_bg_color']} !important; }
.sc_button_hover_style_hover.sc_button_hover_slide_right {		background: linear-gradient(to left,	{$colors['text_hover']} 50%, {$colors['alter_bg_color']} 50%) no-repeat scroll left bottom / 210% 100% {$colors['alter_bg_color']} !important; }
.sc_button_hover_style_hover.sc_button_hover_slide_top {		background: linear-gradient(to bottom,	{$colors['text_hover']} 50%, {$colors['alter_bg_color']} 50%) no-repeat scroll right bottom / 100% 210% {$colors['alter_bg_color']} !important; }
.sc_button_hover_style_hover.sc_button_hover_slide_bottom {		background: linear-gradient(to top,		{$colors['text_hover']} 50%, {$colors['alter_bg_color']} 50%) no-repeat scroll right top / 100% 210% {$colors['alter_bg_color']} !important; }
...
.sc_button_hover_style_alterbd.sc_button_hover_slide_left {		background: linear-gradient(to right,	{$colors['text_hover2']} 50%, {$colors['bg_color']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['bg_color']} !important; }
.sc_button_hover_style_alterbd.sc_button_hover_slide_right {	background: linear-gradient(to left,	{$colors['text_hover2']} 50%, {$colors['bg_color']} 50%) no-repeat scroll left bottom / 210% 100% {$colors['bg_color']} !important; }
.sc_button_hover_style_alterbd.sc_button_hover_slide_top {		background: linear-gradient(to bottom,	{$colors['text_hover2']} 50%, {$colors['bg_color']} 50%) no-repeat scroll right bottom / 100% 210% {$colors['bg_color']} !important; }
.sc_button_hover_style_alterbd.sc_button_hover_slide_bottom {	background: linear-gradient(to top,		{$colors['text_hover2']} 50%, {$colors['bg_color']} 50%) no-repeat scroll right top / 100% 210% {$colors['bg_color']} !important; }

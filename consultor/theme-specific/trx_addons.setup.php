...
------------------------------------------------------------------------------- */
if (!function_exists('consultor_trx_addons_theme_specific_setup1')) {
	add_filter( 'trx_addons_filter_components_editor', 'consultor_trx_addons_theme_specific_components');
	function consultor_trx_addons_theme_specific_components($enable=false) {
		return false;
	}
}

if (!function_exists('consultor_trx_addons_theme_specific_setup1')) {
	add_action( 'after_setup_theme', 'consultor_trx_addons_theme_specific_setup1', 1 );
	add_action( 'trx_addons_action_save_options', 'consultor_trx_addons_theme_specific_setup1', 1 );
	function consultor_trx_addons_theme_specific_setup1() {
		if (consultor_exists_trx_addons()) {
			add_filter( 'trx_addons_cv_enable',					'consultor_trx_addons_cv_enable');
			add_filter( 'trx_addons_demo_enable',				'consultor_trx_addons_demo_enable');
			add_filter( 'trx_addons_filter_edd_themes_market',	'consultor_trx_addons_edd_themes_market_enable');
			add_filter( 'trx_addons_cpt_list',					'consultor_trx_addons_cpt_list');
			add_filter( 'trx_addons_sc_list',					'consultor_trx_addons_sc_list');
			add_filter( 'trx_addons_widgets_list',				'consultor_trx_addons_widgets_list');
		}
	}
}

// CV
if ( !function_exists( 'consultor_trx_addons_cv_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_cv_enable', 'consultor_trx_addons_cv_enable');
	function consultor_trx_addons_cv_enable($enable=false) {
		// To do: return false if theme not use CV functionality
		return CONSULTOR_THEME_FREE
					? false		// Free version
					: false;		// Pro version
	}
}
...
// Demo mode
if ( !function_exists( 'consultor_trx_addons_demo_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_demo_enable', 'consultor_trx_addons_demo_enable');
	function consultor_trx_addons_demo_enable($enable=false) {
...
// EDD Themes market
if ( !function_exists( 'consultor_trx_addons_edd_themes_market_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_edd_themes_market', 'consultor_trx_addons_edd_themes_market_enable')) {
	function consultor_trx_addons_edd_themes_market_enable($enable=false) {
...
// API
if ( !function_exists( 'consultor_trx_addons_api_list' ) ) {
	//Handler of the add_filter('trx_addons_api_list',	'consultor_trx_addons_api_list');
	function consultor_trx_addons_api_list($list=array()) {
...
// CPT
if ( !function_exists( 'consultor_trx_addons_cpt_list' ) ) {
	//Handler of the add_filter('trx_addons_cpt_list',	'consultor_trx_addons_cpt_list');
	function consultor_trx_addons_cpt_list($list=array()) {
...
// Shortcodes
if ( !function_exists( 'consultor_trx_addons_sc_list' ) ) {
	//Handler of the add_filter('trx_addons_sc_list',	'consultor_trx_addons_sc_list');
	function consultor_trx_addons_sc_list($list=array()) {
...
// Widgets
if ( !function_exists( 'consultor_trx_addons_widgets_list' ) ) {
	//Handler of the add_filter('trx_addons_widgets_list',	'consultor_trx_addons_widgets_list');
	function consultor_trx_addons_widgets_list($list=array()) {
...
if ( !function_exists( 'consultor_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'consultor_trx_addons_menu_cache');
	function consultor_trx_addons_menu_cache($list=array()) {
...
if (!function_exists('consultor_trx_addons_localize_script')) {
	...
	function consultor_trx_addons_localize_script($arr) {
		$arr['alter_link_color'] = consultor_get_scheme_color('alter_link');
		return $arr;
	}
}
...
if ( !function_exists( 'consultor_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'consultor_trx_addons_sc_type', 10, 2);
	function consultor_trx_addons_sc_type($list, $sc) {
...
if ( !function_exists( 'consultor_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'consultor_trx_addons_sc_atts', 10, 2);
	function consultor_trx_addons_sc_atts($atts, $sc) {
...
if ( !function_exists( 'consultor_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'consultor_trx_addons_sc_map', 10, 2);
	function consultor_trx_addons_sc_map($params, $sc) {
		...
		$param = array(
			...
			'heading' => esc_html__('Color style', 'consultor'),
			'description' => wp_kses_data( __('Select color style to decorate this block', 'consultor') ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label' => true,
			'value' => array_flip(consultor_get_list_sc_color_styles()),
			'type' => 'dropdown'
		);
...
if ( !function_exists( 'consultor_trx_addons_sow_map' ) ) {
	add_filter( 'trx_addons_sow_map', 'consultor_trx_addons_sow_map', 10, 2);
	function consultor_trx_addons_sow_map($params, $sc) {

		// Param 'color_style'
		$param = array(
			'color_style' => array(
				'label' => esc_html__('Color style', 'consultor'),
				'description' => wp_kses_data( __('Select color style to decorate this block', 'consultor') ),
				'options' => consultor_get_list_sc_color_styles(),
				'default' => 'default',
				'type' => 'select'
			)
		);
		if (in_array($sc, array('trx_sc_button')))
			consultor_array_insert_after($params, 'size', $param);
		else if (in_array($sc, array('trx_sc_action', 'trx_sc_blogger', 'trx_sc_cars', 'trx_sc_courses', 'trx_sc_content', 'trx_sc_dishes',
								'trx_sc_events', 'trx_sc_form',	'trx_sc_googlemap', 'trx_sc_portfolio', 'trx_sc_price', 'trx_sc_promo',
								'trx_sc_properties', 'trx_sc_services', 'trx_sc_team', 'trx_sc_testimonials', 'trx_sc_title',
								'trx_widget_audio', 'trx_widget_twitter')))
			consultor_array_insert_after($params, 'title_align', $param);
		return $params;
	}
}

// Add classes to the shortcode's output
if ( !function_exists( 'consultor_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'consultor_trx_addons_sc_output', 10, 4);
	function consultor_trx_addons_sc_output($output, $sc, $atts, $content) {
		...
		if (in_array($sc, array('trx_sc_action'))) {
			...
			if (!empty($atts['color_style']) && !consultor_is_inherit($atts['color_style']))
...
if ( !function_exists( 'consultor_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'consultor_trx_addons_sc_item_title_tag');
	function consultor_trx_addons_sc_item_title_tag($tag='') {
		return $tag=='h1' ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( !function_exists( 'consultor_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'consultor_trx_addons_sc_item_button_args', 10, 3);
	function consultor_trx_addons_sc_item_button_args($args, $sc, $sc_args) {
...
if ( !function_exists( 'consultor_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title',	'consultor_trx_addons_slider_title', 10, 2 );
	function consultor_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title'])) 
...
if ( !function_exists( 'consultor_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles',	'consultor_trx_addons_sc_googlemap_styles');
	function consultor_trx_addons_sc_googlemap_styles($list) {
		$list['dark'] = esc_html__('Dark', 'consultor');
		return $list;
	}
}
...
if ( !function_exists( 'consultor_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'consultor_trx_addons_tiny_mce_style_formats');
	function consultor_trx_addons_tiny_mce_style_formats($style_formats) {
		...
		if (is_array($style_formats) && count($style_formats)>0 ) {
			foreach ($style_formats as $k=>$v) {
				if ( $v['title'] == esc_html__('List styles', 'consultor') ) {
					$style_formats[$k]['items'][] = array(
								'title' => esc_html__('Arrow', 'consultor'),
								'selector' => 'ul',
				...
				if ( $v['title'] == esc_html__('List styles', 'consultor') ) {
					$style_formats[$k]['items'][] = array(
						'title' => esc_html__('Arrow (Circled)', 'consultor'),
						'selector' => 'ul',
						'classes' => 'trx_addons_list trx_addons_list_arrow_circled'
					);
				}
				if ( $v['title'] == esc_html__('List styles', 'consultor') ) {
					$style_formats[$k]['items'][] = array(
						'title' => esc_html__('Result', 'consultor'),
						'selector' => 'ul',
						'classes' => 'trx_addons_list trx_addons_list_result'
					);
				}
				if ( $v['title'] == esc_html__('Inline', 'consultor') ) {
					$style_formats[$k]['items'][] = array(
						'title' => esc_html__('Accent hovered', 'consultor'),
						'inline' => 'span',
...
if ( !function_exists( 'consultor_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'consultor_filter_allow_override_header_image', 'consultor_trx_addons_allow_override_header_image' );
	function consultor_trx_addons_allow_override_header_image($allow) {
		return consultor_is_team_page() ? false : $allow;
	}
}

// Get thumb size for the team items
if ( !function_exists( 'consultor_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size',	'consultor_trx_addons_thumb_size', 10, 2);
	function consultor_trx_addons_thumb_size($thumb_size='', $type='') {
		if ($type == 'team-default')
			$thumb_size = consultor_get_thumb_size('big');
		return $thumb_size;
	}
}
...
if (!function_exists('consultor_trx_addons_meta_box_fields')) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'consultor_trx_addons_meta_box_fields', 10, 2);
	function consultor_trx_addons_meta_box_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			$mb['email'] = array(
				"title" => esc_html__("E-mail",  'consultor'),
				"desc" => wp_kses_data( __("Team member's email", 'consultor') ),
				"std" => "",
				"details" => true,
				"type" => "text"
			);
		...
		if (defined('TRX_ADDONS_CPT_SERVICES_PT') && $post_type==TRX_ADDONS_CPT_SERVICES_PT) {
			$mb['order_now'] = array(
				"title" => esc_html__("Order now button",  'consultor'),
				"desc" => wp_kses_data( __("Link for Solutions Button (Light type services)", 'consultor') ),
				"std" => "/contacts/",
				"details" => true,
				"type" => "text"
			);

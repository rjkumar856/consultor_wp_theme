	...
	if (!empty($address) || !empty($phone) || !empty($email)) {
		?><div class="contacts_info"><?php
			if (!empty($phone) || !empty($email)) {
				...
				if (!empty($phone)) {
					?><a href="<?php echo esc_attr(trx_addons_get_phone_link($phone)); ?>" class="contacts_phone"><span class="contact_label"><?php echo esc_html__( 'Call Us:', 'consultor' ); ?></span><?php echo wp_kses_data($phone); ?></a><?php
				}
				if (!empty($email)) {
					?><span class="contacts_email"><a href="mailto:<?php echo antispambot($email); ?>"><span class="contact_label"><?php echo esc_html__( 'Write Us:', 'consultor' ); ?></span><?php echo antispambot($email); ?></a></span><?php
				}
				if ($columns) {
					?></div><?php
				}
		...
		if (!empty($address)) {
			...
			?><span class="contacts_address"><span class="contact_label"><?php echo esc_html__( 'Visit Us:', 'consultor' ); ?></span><?php echo str_replace('|', "<br>", $address); ?></span><?php
			if ($columns) {
				?></div><?php
			}
		}
